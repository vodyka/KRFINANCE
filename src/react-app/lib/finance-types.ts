...
export interface BankAccount {
  id: string;
  companyId: string;
  bankCode: string; // código do banco global
  accountName: string;
  initialBalance: number;
  balanceStartDate: string; // YYYY-MM-DD - data de início do saldo
  overdraftLimit?: number; // limite cheque especial
  isDefault?: boolean; // conta padrão que não pode ser excluída
  // Legacy field - will be migrated to bankCode
  bankName?: string;
}
...
export interface Payment {
  ...
  interest?: number; // juros pagos
  fine?: number; // multa paga
}
