...
export default function ContasPage() {
  const { banks, setBanks, getBankBalance, getBankLogo, activeCompanyId } = useFinanceData();
  ...
  const handleSave = () => {
    ...
    if (editing) {
      setBanks(prev => {
        // If setting this bank as default, unset all other defaults for this company
        if (form.isDefault) {
          return prev.map(b => 
            b.id === editing.id 
              ? entry 
        ...
        return prev.map(b => b.id === editing.id ? entry : b);
      });
    } else {
      setBanks(prev => {
        // If setting new bank as default, unset all other defaults for this company
        if (form.isDefault) {
          return [
            ...prev.map(b => b.companyId === activeCompanyId ? { ...b, isDefault: false } : b),
            entry
          ];
        }
        return [...prev, entry];
  ...
  const toggleDefault = (id: string) => {
    setBanks(prev => prev.map(b => {
      ...
      if (b.companyId === activeCompanyId && b.isDefault) {
